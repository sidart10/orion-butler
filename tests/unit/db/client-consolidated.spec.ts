/**
 * Tests for Consolidated OrionDatabase Client
 * Database Initialization Fix - Phase 0.4
 *
 * TDD Phase: RED - Write failing tests first
 *
 * Tests the consolidated OrionDatabase singleton from src/db/client.ts
 */
...
describe('OrionDatabase (Consolidated)', () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  describe('State Management', () => {
    it('should start in disconnected state', async () => {
      const { OrionDatabase } = await import('@/db/client');
  ...
  describe('Initialization', () => {
    ...
    it('should not call init twice if already connected', async () => {
      // Mock full Tauri environment for this test
    ...
    it('should return config with tablesCreated after successful init', async () => {
      vi.doMock('@tauri-apps/plugin-sql', () => ({
        default: {
          load: vi.fn().mockResolvedValue({
            execute: vi.fn().mockResolvedValue(undefined),
