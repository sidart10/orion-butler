 ...
 *
 * Architecture:
 * - Uses consolidated init.ts for PRAGMA + table creation
 * - Uses Tauri IPC for directory setup (db_ensure_dir)
 * - Uses @tauri-apps/plugin-sql for SQLite access
 */

import { invoke } from '@tauri-apps/api/core';
import type { DbHealthStatus, DbConnectionState, DbConfigResult, IDatabase } from './types';
import { initializeDatabaseWithConnection, verifyConfig } from './init';

// Database interface from tauri-plugin-sql (extends IDatabase with close)
interface TauriDatabase extends IDatabase {
  close(): Promise<void>;
}
...
export class OrionDatabase {
   ...
   * Ensures PRAGMA settings are applied, tables created, and verified
   */
  ...
  private async _doInit(): Promise<void> {
    ...
    try {
      ...
      // Use consolidated initialization (PRAGMAs + tables)
      this.config = await initializeDatabaseWithConnection(this.db);
