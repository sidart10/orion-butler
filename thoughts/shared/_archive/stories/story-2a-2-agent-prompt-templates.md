...
**Story Key:** 2a-2-agent-prompt-templates
...
**Agent Inventory (26 total - per adversarial review):**
...
**Adaptable Agents (10) - Keep with prompt modifications:**
...
**Optional Agents (4) - Available but not core:**
...
**NOT IN SCOPE (excluded from Orion):**
- BMAD agents (analyst, dev, pm, sm, tea, tech-writer, ux-designer, quick-flow-solo-dev) - different workflow methodology
- Code-only agents (braintrust-analyst, research-codebase, session-analyst, surveyor, judge, profiler, review-agent, validate-agent, atlas, liaison) - CC-specific
...
- [ ] 20 reusable/adaptable agents load correctly from `.claude/agents/`
...
```typescript
// Orion Agent System - 26 agents total (per adversarial review)
// 6 Orion-specific + 6 reusable + 10 adaptable + 4 optional
...
// Adaptable agents (10) - Keep with Orion-specific prompt modifications
...
```
- [ ] 4.2 Verify `ALL_AGENTS.length === 26`

### Task 5: Write Tests (AC: #1, #2)

- [ ] 5.1 Unit test: Template loading works for all 26 agents
...
ADAPTABLE (10)                    OPTIONAL (4)
+------------------+              +------------------+
| scout → navigator|              | aegis (security) |
| architect → plan |              | arbiter (rules)  |
| sleuth → trouble |              | kraken (complex) |
| spark → quickfix |              | pathfinder       |
| critic → review  |              +------------------+
| herald → report  |
| plan-agent       |
| debug-agent      |
...
- Code-only agents (braintrust-analyst, research-codebase, session-analyst, surveyor, judge, profiler, review-agent, validate-agent, atlas, liaison)
...
|----|------|----------|-----------------|
| 2.2.1 | Unit | All 26 agent templates parse | No parse errors |
...
describe('Story 2.2: Agent Prompt Templates', () => {
  ...
  test('2.2.1 - all 26 agent templates load without errors', async () => {
    const errors: string[] = [];
    ...
    expect(ALL_AGENTS.length).toBe(26);
  });
