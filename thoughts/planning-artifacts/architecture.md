...
### 6.5 Claude Agent SDK Built-in Tools
...
#### 6.5.3 Computer Use Tool (Future)
...
Beyond Composio, Orion can extend capabilities via custom MCP servers.

```typescript
// agent-server/src/mcp-config.ts

import { McpServerConfig } from '@anthropic-ai/claude-agent-sdk';

export const mcpServers: McpServerConfig[] = [
  // Composio for Gmail, Calendar, Slack
  {
    type: 'http',
    name: 'composio',
  ...
  // Local filesystem MCP (if needed beyond text_editor)
  {
    type: 'stdio',
    name: 'filesystem',
    command: 'npx',
  ...
  // SQLite MCP for direct database queries
  {
    ...
    name: 'sqlite',
    command: 'npx',
    args: ['-y', '@anthropic-ai/mcp-server-sqlite', '--db-path', './orion.db'],
  },
];
...
### 6.7 Prompt Caching (Cost Optimization)
