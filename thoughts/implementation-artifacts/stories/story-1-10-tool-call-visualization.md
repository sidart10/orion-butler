...
- [ ] **Task 1: Create ToolCard Component** (AC: 1, 2, 3, 4, 5, 8, 9)
  - [ ] 1.1 Create `src/components/chat/ToolCard.tsx`:
    ```typescript
    ...
    ```
  - [ ] 1.2 Implement collapsed/expanded states with animation
  - [ ] 1.3 Add status indicators (spinner, checkmark, error)
  - [ ] 1.4 Implement XSS protection via escapeHtml
  - [ ] 1.5 Add accessibility attributes (aria-expanded, focusable)

- [ ] **Task 2: Create ToolCardList Component** (AC: 6, 10)
  - [ ] 2.1 Create `src/components/chat/ToolCardList.tsx`:
    ```typescript
    ...
    ```
  - [ ] 2.2 Render tools in sequential order
  - [ ] 2.3 Add proper list semantics for accessibility

- [ ] **Task 3: Update Chat Store for Tool Tracking** (AC: 1, 4, 5, 6)
  - [ ] 3.1 Verify/update `src/stores/chatStore.ts`:
    ```typescript
    ...
    ```
  - [ ] 3.2 Add input/output tracking to ToolStatus
  - [ ] 3.3 Add startTime/endTime for duration calculation
  - [ ] 3.4 Add clearTools action for conversation reset

- [ ] **Task 4: Integrate Tool Events with Streaming** (AC: 1, 4, 5, 10)
  - [ ] 4.1 Update streaming handler in `src/components/chat/ChatPanel.tsx`:
    ```typescript
    ...
    case 'tool_error':
      ...
      break;
    ```
  - [ ] 4.2 Calculate duration from startTime/endTime
  - [ ] 4.3 Handle interleaved text and tool events

- [ ] **Task 5: Integrate ToolCardList into Chat Flow** (AC: 10)
  - [ ] 5.1 Update `src/components/chat/ChatPanel.tsx` or `MessageList.tsx`:
    ```typescript
    ...
    ```
  - [ ] 5.2 Position tool cards correctly in message flow
  - [ ] 5.3 Clear tools appropriately (on new conversation or response complete)

- [ ] **Task 6: Style Tool Cards with Orion Design System** (AC: 8)
  - [ ] 6.1 Apply design system tokens:
  ...
  - [ ] 6.2 Ensure compact height in collapsed state (max 60px)
  - [ ] 6.3 Add gold left border accent for visual hierarchy

- [ ] **Task 7: Unit Tests for ToolCard** (AC: 7, 8)
  - [ ] 7.1 Create `tests/unit/components/chat/ToolCard.test.tsx`:
    ```typescript
    ...
    ```
  - [ ] 7.2 Test XSS protection with malicious payloads
  - [ ] 7.3 Test all status states (running, complete, error)
  - [ ] 7.4 Test keyboard accessibility

- [ ] **Task 8: E2E Tests for Tool Visualization** (AC: 1, 3, 6)
  - [ ] 8.1 Create `tests/e2e/tool-cards.spec.ts`:
    ```typescript
...
`,
    ...
    ```
  - [ ] 8.2 Test tool card appearance on invocation
  - [ ] 8.3 Test expand/collapse interaction
  - [ ] 8.4 Test multiple tool cards stacking
  - [ ] 8.5 Add visual regression snapshots

- [ ] **Task 9: Replace ToolStatusBar with ToolCardList** (AC: 10)
  - [ ] 9.1 Update ChatPanel to use ToolCardList instead of ToolStatusBar
  - [ ] 9.2 Remove or deprecate ToolStatusBar component if no longer needed
  - [ ] 9.3 Ensure backward compatibility during transition
...
| E2E | Playwright | `tests/e2e/tool-cards.spec.ts` | Visual regression |
