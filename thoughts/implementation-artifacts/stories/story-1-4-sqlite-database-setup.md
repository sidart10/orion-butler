...
- [ ] **Task 1: Database Path & Directory Setup** (AC: 1)
  - [ ] 1.1 Create utility function to determine database path:
    ```typescript
    // src/lib/db/path.ts
    ...
    ```
  - [ ] 1.2 Create directory if it doesn't exist with `fs.mkdirSync(dir, { recursive: true })`
  - [ ] 1.3 Verify correct permissions (0o700 for directory, 0o600 for db file)
  - [ ] 1.4 Handle cross-platform fallback for non-macOS development (./data folder)

- [ ] **Task 2: Database Initialization Module** (AC: 1, 2, 7)
  - [ ] 2.1 Install better-sqlite3: `pnpm add better-sqlite3 @types/better-sqlite3`
  - [ ] 2.2 Create database initialization module:
    ```typescript
    // src/lib/db/index.ts
    ...
    ```
  - [ ] 2.3 Add database close function for graceful shutdown
  - [ ] 2.4 Export typed database instance

- [ ] **Task 3: Schema Version Tracking** (AC: 6)
  - [ ] 3.1 Create schema_version table:
    ```sql
    ...
    ```
  - [ ] 3.2 Create function to get current schema version
  - [ ] 3.3 Create function to set schema version after migration

- [ ] **Task 4: Migration Framework** (AC: 6)
  - [ ] 4.1 Create migrations directory: `src/lib/db/migrations/`
  - [ ] 4.2 Create migration runner:
    ...
    ```
  - [ ] 4.3 Ensure migrations run within transactions
  - [ ] 4.4 Add logging for migration progress

- [ ] **Task 5: Initial Migration - Conversations & Messages** (AC: 3, 4)
  - [ ] 5.1 Create first migration file:
    ```typescript
    ...
    ```
  - [ ] 5.2 Verify all column types match architecture spec
  - [ ] 5.3 Add CHECK constraints for enum-like fields (role, feedback)

- [ ] **Task 6: ID Generation Utilities** (AC: 3, 4)
  - [ ] 6.1 Create ID generation module:
    ```typescript
    // src/lib/db/ids.ts
    ...
    ```
  - [ ] 6.2 Ensure IDs are URL-safe and unique

- [ ] **Task 7: Type Definitions** (AC: 3, 4)
  - [ ] 7.1 Create TypeScript types for database entities:
    ```typescript
    // src/lib/db/types.ts
    ...
    ```
  - [ ] 7.2 Add type guards for JSON fields

- [ ] **Task 8: Basic CRUD Operations** (AC: 3, 4, 5)
  - [ ] 8.1 Create conversation repository:
    ```typescript
  ...
  - [ ] 8.2 Create message repository:
    ...
    ```
  - [ ] 8.3 Use prepared statements for performance

- [ ] **Task 9: Integration with Tauri** (AC: 1, 5)
  - [ ] 9.1 Create Tauri command for database initialization:
    ```typescript
    // src-tauri/src/commands/db.rs (if using Rust)
    // OR src/lib/tauri/commands.ts (if using JS)
    ```
  - [ ] 9.2 Initialize database on app startup (before frontend loads)
  - [ ] 9.3 Ensure database is closed on app quit

- [ ] **Task 10: Unit Tests** (AC: 1, 2, 3, 4, 5, 6, 7)
  - [ ] 10.1 Create test setup with in-memory database option:
    ```typescript
    // tests/db/setup.ts
    ...
    ```
  - [ ] 10.2 Test database path resolution
  - [ ] 10.3 Test WAL mode activation
  - [ ] 10.4 Test foreign key enforcement
  - [ ] 10.5 Test conversation CRUD operations
  - [ ] 10.6 Test message CRUD operations
  - [ ] 10.7 Test cascade delete (messages deleted when conversation deleted)
  - [ ] 10.8 Test schema migration idempotency

- [ ] **Task 11: Integration Tests** (AC: 1, 2, 5)
  - [ ] 11.1 Test database file created at correct path
  - [ ] 11.2 Test WAL mode is active: `PRAGMA journal_mode` returns 'wal'
  - [ ] 11.3 Test data persistence across restarts (using temp directory)
  - [ ] 11.4 Test directory creation if parent doesn't exist
