...
- [ ] **Task 1: Extend Claude Client for Streaming** (AC: 1, 3)
  - [ ] 1.1 Modify `agent-server/src/services/claude-client.ts`:
    ```typescript
    ...
    ```
  - [ ] 1.2 Add streaming method to existing ClaudeClient class
  - [ ] 1.3 Implement stream event handling for text deltas
  - [ ] 1.4 Handle thinking deltas for extended thinking support (future)

- [ ] **Task 2: Create SSE Streaming Endpoint** (AC: 1)
  - [ ] 2.1 Update `agent-server/src/routes/stream.ts`:
    ```typescript
...
Be helpful, concise, and proactive. Ask clarifying questions when needed. Remember context from the conversation.`;
    ...
    ```
  - [ ] 2.2 Replace placeholder SSE endpoint from Story 1.5
  - [ ] 2.3 Implement proper SSE event formatting
  - [ ] 2.4 Add error handling with appropriate error codes
  - [ ] 2.5 Support both GET (query params) and POST (body) for different payload sizes

- [ ] **Task 3: Register Stream Routes** (AC: 1)
  - [ ] 3.1 Update `agent-server/src/index.ts`:
    ```typescript
    ...
    ```
  - [ ] 3.2 Register stream routes in Express app

- [ ] **Task 4: Create Streaming Types** (AC: 1, 8)
  - [ ] 4.1 Create `src/types/streaming.ts`:
    ```typescript
    ...
    ```
  - [ ] 4.2 Define all stream event types
  - [ ] 4.3 Add streaming state interface for tracking

- [ ] **Task 5: Create Streaming Service** (AC: 1, 3, 8)
  - [ ] 5.1 Create `src/lib/services/streamingService.ts`:
    ```typescript
    ...
    ```
  - [ ] 5.2 Implement SSE parsing from fetch response
  - [ ] 5.3 Handle all event types with callbacks
  - [ ] 5.4 Add abort capability for cancellation

- [ ] **Task 6: Create Tauri IPC Stream Forwarding** (AC: 8)
  - [ ] 6.1 Create `src-tauri/src/stream_forwarder.rs`:
    ```rust
    ...
    ```
  - [ ] 6.2 Create Rust command for starting stream
  - [ ] 6.3 Implement SSE parsing in Rust
  - [ ] 6.4 Emit Tauri events for each stream event
  - [ ] 6.5 Register command in Tauri plugin system

- [ ] **Task 7: Update Chat Store for Streaming** (AC: 1, 2, 4, 6)
  - [ ] 7.1 Extend `src/stores/chatStore.ts`:
    ```typescript
    ...
    ```
  - [ ] 7.2 Add streaming state variables
  - [ ] 7.3 Implement `sendMessageStreaming` action
  - [ ] 7.4 Implement `appendToStream` with first token tracking
  - [ ] 7.5 Implement `completeStream` with database persistence
  - [ ] 7.6 Add stream cancellation capability

- [ ] **Task 8: Create Typing Indicator Component** (AC: 2)
  - [ ] 8.1 Create `src/components/chat/TypingIndicator.tsx`:
    ```typescript
    ...
    ```
  - [ ] 8.2 Style with Orion Design System (gold color, editorial typography)
  - [ ] 8.3 Add bounce animation for dots
  - [ ] 8.4 Show only before first token, hide after

- [ ] **Task 9: Create Streaming Text Component** (AC: 4)
  - [ ] 9.1 Create `src/components/chat/StreamingText.tsx`:
    ```typescript
    ...
    ```
  - [ ] 9.2 Render text with whitespace preservation
  - [ ] 9.3 Add streaming cursor indicator
  - [ ] 9.4 Optimize re-renders during rapid updates

- [ ] **Task 10: Implement Auto-Scroll** (AC: 5)
  - [ ] 10.1 Create `src/hooks/useAutoScroll.ts`:
    ```typescript
    ...
    ```
  - [ ] 10.2 Implement auto-scroll on new content
  - [ ] 10.3 Detect user manual scroll to pause auto-scroll
  - [ ] 10.4 Reset scroll state when streaming completes

- [ ] **Task 11: Update Message List for Streaming** (AC: 4, 5, 7)
  - [ ] 11.1 Update `src/components/chat/MessageList.tsx`:
    ```typescript
    ...
    ```
  - [ ] 11.2 Integrate auto-scroll hook
  - [ ] 11.3 Show typing indicator at correct time
  - [ ] 11.4 Add "scroll to bottom" button when user scrolled up
  - [ ] 11.5 Pass streaming state to message bubbles

- [ ] **Task 12: Update Message Bubble for Streaming** (AC: 4)
  - [ ] 12.1 Update `src/components/chat/MessageBubble.tsx`:
    ```typescript
    ...
    ```
  - [ ] 12.2 Use StreamingText component for streaming messages
  - [ ] 12.3 Apply Orion Design System chat styles

- [ ] **Task 13: Update Chat Input for Streaming** (AC: 1, 6)
  - [ ] 13.1 Update `src/components/chat/ChatInput.tsx`:
    ```typescript
    ...
    ```
  - [ ] 13.2 Switch from `sendMessage` to `sendMessageStreaming`
  - [ ] 13.3 Add cancel button during streaming
  - [ ] 13.4 Disable input during streaming

- [ ] **Task 14: Performance Monitoring** (AC: 3, 7)
  - [ ] 14.1 Create `src/lib/performance/streamMetrics.ts`:
    ```typescript
    ...
    ```
  - [ ] 14.2 Track first token latency
  - [ ] 14.3 Track total tokens and tokens/second
  - [ ] 14.4 Log performance metrics for debugging

- [ ] **Task 15: Unit Tests** (AC: 2, 3, 4)
  - [ ] 15.1 Test typing indicator state machine:
    ```typescript
    ...
    ```
  - [ ] 15.2 Test SSE event parsing
  - [ ] 15.3 Test auto-scroll hook
  - [ ] 15.4 Test streaming text component

- [ ] **Task 16: Integration Tests** (AC: 1, 6)
  - [ ] 16.1 Test SSE endpoint:
    ```typescript
    ...
    ```
  - [ ] 16.2 Test stream completion persists to database
  - [ ] 16.3 Test error handling during stream

- [ ] **Task 17: E2E Tests** (AC: 1, 3, 4, 7)
  - [ ] 17.1 Create Playwright tests:
    ```typescript
    ...
    ```
  - [ ] 17.2 Test typing indicator lifecycle
  - [ ] 17.3 Test smooth streaming rendering
  - [ ] 17.4 Test long response handling
  - [ ] 17.5 Test cancel functionality
...
| E2E | Playwright | `tests/e2e/streaming.spec.ts` | Full app testing |
