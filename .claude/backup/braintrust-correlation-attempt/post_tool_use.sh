...
# Extract agent type from Task tool and correlate child session
AGENT_TYPE=""
CHILD_SESSION_ID=""
TASK_SPAN_ID=""
if [ "$TOOL_NAME" = "Task" ]; then
    ...
    # Get the task_span_id that was generated by PreToolUse
    TASK_SPAN_ID=$(get_session_state "$SESSION_ID" "pending_task_span_id")

    if [ -n "$TASK_SPAN_ID" ]; then
        # Read child_session_id from correlation record (written by sub-agent's SessionStart)
        # This is race-free because:
        # 1. PreToolUse creates the record BEFORE sub-agent starts
        # 2. SessionStart updates it with child_session_id DURING sub-agent
        # 3. PostToolUse reads it AFTER sub-agent finishes
        CHILD_SESSION_ID=$(get_correlation_child_session "$TASK_SPAN_ID")

        if [ -n "$CHILD_SESSION_ID" ]; then
            debug "Task child_session_id: $CHILD_SESSION_ID (from correlation record)"
            # Clean up the correlation record after consuming it
            remove_correlation_record "$TASK_SPAN_ID"
        else
            debug "No child_session_id in correlation record for task_span_id=$TASK_SPAN_ID"
        fi

        # Clear the pending task span id
        set_session_state "$SESSION_ID" "pending_task_span_id" ""
    else
        debug "No pending_task_span_id found for this Task"
    fi
fi
...
EVENT=$(jq -n \
    ...
    '{
        ...
        metadata: ({
          ...
          + (if $child_session_id != "" then {child_session_id: $child_session_id} else {} end)),
        span_attributes: {
            name: $name,
            type: "tool"
        }
